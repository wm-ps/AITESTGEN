...
- [ ] Task 4: Wire the Temporal Python SDK end-to-end (AC: 4)
  ...
  - [ ] Define one trivial no-op Workflow in `packages/workflows`, containing no I/O (AD-2) — this is the same workflow shell that Story 3.2's `GenerationWorkflow` will later extend, so name it/structure it so it can grow into `GenerationWorkflow` rather than being throwaway code
...
- **Scope discipline:** Do not implement `Organization`, `Application`, sign-in, or any real domain entity beyond the one minimal proof entity in Task 2 — those are Story 1.2 and 1.3's job. Do not implement any `AIProvider`/`DeliveryAdapter`/`SecretsClient` concrete adapter — those belong to Epics 2, 5, and 1 (Story 1.3) respectively. A scaffold story that quietly does a later story's work creates merge conflicts and duplicate-decision risk for whoever picks up 1.2 next.
...
- [Source: _bmad-output/planning-artifacts/implementation-readiness-report-2026-07-13.md — Story 3.2's `GenerationWorkflow` reference confirmed non-blocking because "a workflow shell already exists from Story 1.1 (a trivial no-op workflow)"]
