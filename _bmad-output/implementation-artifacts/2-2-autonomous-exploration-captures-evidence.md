...
1. **Given** a running Discovery Run with a configured scope, **when** `DiscoveryActivity` navigates pages, exercises UI actions and forms, and invokes APIs within that scope, **then** each captured page, action, form, API call, and state transition is written as an `Evidence` row tagged with `discovery_run_id`. [Source: epics.md#Story 2.2; FR-6; architecture#AD-8]
...
- [ ] Task 3: Build the real autonomous exploration loop in `DiscoveryActivity` (AC: 1, 2)
  - [ ] Replace Story 2.1's stub with real behavior: establish a session using the Application's stored credentials (via `SecretsClient`, from Stories 1.3/1.4), then navigate pages reachable within `Application.discovery_scope` (null = unrestricted, per Story 1.5), exercise UI actions/forms with generic/synthetic input data, and capture invoked API calls via Playwright's request/response interception
  ...
  - [ ] **Do not implement FR-7's stop-condition logic here** (exhaustive-traversal detection, time-budget cutoff) — that is Story 2.3's job, and it is also what actually sets `DiscoveryRun.status` to `complete`/`incomplete`. This story only needs the capture loop to be boundable for testing purposes — use a simple, clearly-marked placeholder (e.g. a max-iteration safety cap) rather than building real stop-condition detection prematurely; Story 2.3 replaces this placeholder with the real rules
- [ ] Task 4: Build the Discovery Progress live-feed list (AC: 3)
  ...
  - [ ] No push channel is architecturally required — the one delivery-mechanism choice the Architecture Spine explicitly left to the implementer was the review-queue pending-count badge (Deferred section), but the same "boring technology" reasoning applies here: client-side polling of a simple "recent Evidence for this run" read endpoint is a reasonable default. A WebSocket/SSE push channel is a valid alternative but not required by any AC
...
- **Scope boundary with Story 2.3, restated for clarity:** this story makes the capture loop *boundable*; Story 2.3 makes it *correct* (real exhaustive/time-budget detection, and the actual `DiscoveryRun.status` transition). Don't let this story's placeholder stopping point leak into being treated as the real FR-7 implementation during code review.
...
- **Depends on Stories 1.1–1.5 and 2.1 being actually implemented**, not just created — all remain `ready-for-dev` as of this story's creation, and `git log` shows only the initial BMad-tooling commit. This story specifically needs Story 2.1's `DiscoveryRun`/`DiscoveryWorkflow`/stub `DiscoveryActivity`, and Stories 1.3/1.4's `SecretsClient`-stored Application credentials.
...
Story 2.1 remains `ready-for-dev`, so there is no implemented stub `DiscoveryActivity` yet to build on top of — check its File List once implemented for the exact stub signature/dispatch shape before starting Task 3. Stories 1.1–1.5 are also all still `ready-for-dev`; this story specifically needs 1.3/1.4's `SecretsClient`-backed Application credentials to be resolvable.
