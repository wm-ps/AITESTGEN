...
*Reverted from `done` to `in-progress` 2026-07-21 per `sprint-change-proposal-2026-07-21.md` (CR-2) — AC 3 (the raw live-feed) is walked back to a data-capture-only behavior (no longer user-facing); gains AC 10 (stage transitions) and Task 6 below. Everything else on this story is unchanged.*
...
- [x] ~~Task 4: Build the Discovery Progress live-feed list (AC: 3)~~ **`[SUPERSEDED 2026-07-21]`** AC3 no longer requires a user-facing live feed — the endpoint/component this task built (`GET /discovery-runs/{id}/captures`, `CaptureLiveFeed.tsx`) still exist and are harmless, but Story 2.7 stops rendering `CaptureLiveFeed` and replaces it with business-stage progress. Do not extend this task's live-feed further; see Story 2.7 for the current user-facing progress work.
...
- [x] **`[ADDED 2026-07-21]`** Task 6: Stage transitions in `discovery_activity` (AC: 10)
  - [x] `discovery_activity` (`apps/workers/discovery/src/discovery_worker/activities.py`) now sets `discovery_run.stage = "authenticating"` immediately before `establish_session(...)`, then `"discovering"` immediately after it succeeds, before `run_discovery_crawl(...)` starts. Each write is committed on its own, ahead of the existing session-establishment flow.
  - [x] Did not touch the existing `discovery_run.status = "complete"` write.
  - [x] Added an assertion to the existing live-crawl integration test (`test_discovery_activity_captures_the_application_model_against_live_target`) confirming `stage == "discovering"` after a successful run — proving both transitions fired (order is structurally guaranteed: `authenticating` is set, then `establish_session` must succeed, then `discovering` is set, before any crawl activity begins).

### Task 6 Completion Notes (2026-07-21)

- Verified via `apps/workers/discovery/tests/test_discovery_activity_integration.py` against a real local target app (Postgres/Vault/MinIO), part of the full 71-test backend suite run for this Change Request.
...
- [Source: _bmad-output/planning-artifacts/sprint-change-proposal-2026-07-21.md — CR-2, AC3 walk-back]
- [Source: _bmad-output/implementation-artifacts/2-7-business-oriented-import-progress-display.md — supersedes this story's live-feed as the user-facing progress display]
