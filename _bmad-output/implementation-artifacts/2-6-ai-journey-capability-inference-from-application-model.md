...
- **AD-13's `identity_key` is load-bearing for any future cross-run re-discovery dedup work** (formerly scoped as Story 3.5, cut in full for V1 2026-07-21). `[FIX]` This pass makes an explicit, documented decision that step *order* does not participate in the key — only membership does. If that future work later needs order-sensitivity too, that's a deliberate schema/key change to make then, not an oversight to inherit silently.
...
- **Capability dedup is per-`InferenceActivity`-call/per-Application, not cross-run** — a second
  ...
  is a reasonable minimal behavior, not full cross-run re-discovery dedup refinement (formerly
  scoped as Story 3.5, cut in full for V1 2026-07-21 — see epics.md#Epic 3).
