...
3. `[CORRECTED 2026-07-29 — no longer true, see Dev Notes]` ~~Given local development or CI, when the same `object_store.py` code runs, then it continues to target the existing docker-compose MinIO service unchanged~~ — **`docker-compose.yml` no longer runs a local MinIO service at all.** Sometime after this story was implemented (not attributed to any story — `docker-compose.yml` isn't in this story's own File List below), `docker-compose.yml`'s `minio` block was removed and replaced with a comment stating local dev now points at real AWS S3 too, via the same `AWS_S3_BUCKET`/`AWS_REGION`/AWS credentials in `.env` that production uses. `object_store.py`'s backend-selection code (Task 1, unchanged) already supported this correctly — `AWS_S3_BUCKET` set → S3, unset → MinIO — so no application code needed to change; only the local-dev *infrastructure* decision changed. [Source: epics.md#Story 2.8; current `docker-compose.yml` header comment]

**Notes (from epics.md):** `[STALE — see AC 3 correction]` Local-dev behavior was originally intentionally out of scope — MinIO in `docker-compose.yml` was meant to remain the local/CI substitute. That decision was superseded after this story shipped; local dev now uses the same real S3 bucket as production. Only the production (`ops/k8s/`) deployment and its ConfigMap/Secret wiring were this story's own actual scope.
...
- **`[STALE, 2026-07-29]` Local dev was explicitly out of scope at the time this story was written** — `docker-compose.yml`'s `minio` service block was there and this story's instruction was "do not remove or modify" it. That has since changed: `docker-compose.yml` no longer runs MinIO at all; its current header comment reads "Object storage (Story 2.2/2.8) points at real AWS S3 even for local dev (AWS_S3_BUCKET/AWS_REGION/AWS creds in .env) — no local MinIO container." This change isn't in this story's own File List, so it happened in a later, unattributed edit — flagging it here rather than silently leaving this Dev Note contradicting the actual infrastructure. `object_store.py` needed no change either way, since its `AWS_S3_BUCKET`-presence check already made this swap purely a config/infra decision (see AC 3's correction above). One practical consequence: `test_discovery_activity_integration.py`'s object-storage-availability test gate had to be corrected in a separate session to check S3 (not MinIO) reachability when `AWS_S3_BUCKET` is set — see Story 2.2's Change Log, 2026-07-29 entry.
...
- Unchanged by this story (explicitly, by design): `apps/workers/discovery/src/discovery_worker/{crawler,session,activities}.py`, `apps/workers/discovery/tests/test_discovery_activity_integration.py`'s existing MinIO-path tests. `[STALE, 2026-07-29]` `docker-compose.yml` was also unchanged *by this story*, but was later modified (not attributed to any story) to remove MinIO entirely — see Dev Notes correction above.
...
- `[STALE, 2026-07-29 — see AC 3 correction]` ~~Local dev/CI is unaffected: `docker-compose.yml`, `AWS_S3_BUCKET` unset by default, MinIO path exercised exactly as before.~~ No longer accurate — local dev/CI now also point at real S3 (`AWS_S3_BUCKET` set via `.env`), per a later, unattributed `docker-compose.yml` change. Not a regression: `object_store.py`'s backend selection already handled this correctly without modification; only the test suite's environment-availability check needed a fix (Story 2.2, 2026-07-29).
- **`[ADDED 2026-07-29]` Re-verified for this session's Epic 2 closure pass.** Status remains `review`, not `done` — the only outstanding work is still the genuine operator action this story's own Completion Notes already named (applying `ops/k8s/*.yaml` and creating `aitestgen-secrets` on the live EKS cluster), which requires cluster access this environment doesn't have. Confirmed the S3 code path itself continues to work: the object-storage integration tests in `test_discovery_activity_integration.py` (Story 2.2) now pass for real against the production `wmps-ai-testgen` bucket as part of this session's broader Epic 2 verification (57 passed, 0 skipped, `apps/workers/discovery/` full suite). AC 3's text and this section's stale local-dev claim corrected above; no code change needed.
