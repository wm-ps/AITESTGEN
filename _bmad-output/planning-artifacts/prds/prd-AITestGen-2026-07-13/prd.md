...
Once test generation is complete, platform lets a reviewer download all current Test Suites for an Application as a single, complete, ready-to-run Playwright project archive (zip). Generated tests are organized by **suite-level folders** — each current Test Suite becomes its own folder under `tests/`, named after its Journey, containing that suite's generated test files — so the downloaded project retains the same logical grouping shown in the product (Generate Suite / Test Suites Generated). `[CORRECTED 2026-07-27]` The project is **Python** (`pytest` + `pytest-playwright`), not Node/TypeScript — `TestAsset.code` is generated as Python `playwright.sync_api` code (see `HostedAIProvider.generate_playwright`'s prompt), and no code-language conversion step exists or is being added. The archive includes a generated `requirements.txt` (`pytest`, `pytest-playwright`), a `conftest.py`/`pytest.ini`, empty `fixtures/` and `utils/` scaffold folders, and a `README.md` explaining setup, so it is runnable standalone after `pip install -r requirements.txt && playwright install`, either as a whole (`pytest`) or per suite (`pytest tests/<suite-folder>`).
...
**Out of Scope:** No incremental/per-suite download as a separate action — always the full project containing all current suites for the Application in one archive. No server-side or cloud test execution. No delivery to a customer's Git host or CI system. No authentication handling for the downloaded project against the target Application (fixtures/utils are empty scaffolding, not credential wiring). No TypeScript/Node output — that would require converting AI-generated Python to TypeScript, a second code-generation step not built here (see Architecture Deferred).
