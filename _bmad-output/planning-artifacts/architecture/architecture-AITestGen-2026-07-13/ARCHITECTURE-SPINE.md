...
| **CI Delivery** `[DEFERRED POST-V1 — 2026-07-15]`<br>`apps/workers/generation` (`CIDeliveryActivity`), `packages/delivery_adapters` | Push a Test Asset into the customer's repo (PR or direct commit) | FR-19–20 | `TestAsset` + Application's configured Git host | PR/commit on the customer's Git host | `DeliveryAdapter` port (AD-4), AD-9 | Port contract retained (no harm keeping it ready); do not build against FR-19-20 until the real execution/delivery mechanism is designed — see Deferred section |
| **CI Instructions** `[DEFERRED POST-V1 — 2026-07-15]`<br>`packages/ci_instructions` | Produce the manual pipeline-wiring template for the Application's CI system | FR-21 | Application's configured CI system | Instructions/template surfaced in `apps/web` | none — pure function of CI system | Port contract retained; not scheduled for V1 build — see Deferred section |
| **Analytics** `[MOSTLY DEFERRED POST-V1 — 2026-07-15]`<br>`apps/api` analytics endpoints, `apps/web` | Journey step/evidence detail (retained, served inline from the discovery-review flow, not a standalone screen); Capability Map, coverage analytics, executive dashboard (deferred) | FR-23 retained; FR-22, FR-24-25 deferred | `current=true` Journey/Capability/TestAsset rows, Organization scope | Read views only | `packages/domain` (read-only) | A new dashboard view can never corrupt Trusted Knowledge Model state — Analytics has no write path into `packages/domain` |
| **Tenancy**<br>`packages/domain` (`Organization`), `apps/api` scoping | Owns the Organization boundary every other module's queries run through | platform auth (FR-25's dashboard is deferred, but the tenant boundary itself is not — see note below) | Authenticated user session | Query-layer filter applied platform-wide (AD-12) | none — foundational | A data-isolation bug is fixable in exactly one place — the central scoping mechanism — never in N separate endpoints |
| **Deployment topology** `[DEFERRED POST-V1 — 2026-07-15]` | SaaS/on-prem infra, Temporal hosting, AI-provider-mode configuration | FR-26–27 | — | — | `AIProvider` port keeps this addable later | Deferred — see Deferred section (deferral reason updated 2026-07-15: no confirmed UI entry point, not just infra complexity) |

## Deferred

- **`[NEW — 2026-07-15]` Test-suite execution mechanism**: Generate Suite's `Run immediately`/`Schedule for later`/`Save without running` controls are a confirmed UI placeholder only (user-stated: "currently it is placeholder, we will tag the integration later"). **No architecture decision is made here.** Specifically: do not read "Run immediately" as authorizing the platform to execute generated tests itself — that would silently contradict the existing "no CI read-back channel" stance (FR-24 consequence, itself now deferred but not reversed) without a real decision ever having been made. Revisit once the mechanism is actually specified; it may turn out to be a rename of the existing CI Delivery module (below), a genuinely new test-runner capability, or something else entirely.
- **`[UPDATED 2026-07-15]` CI/CD delivery** (FR-19-21): deferred post-V1 — the Connect to CI/CD screen this depended on is cut from the current reference prototype's IA (see `sprint-change-proposal-2026-07-15.md`). The `DeliveryAdapter`/`CIInstructionsGenerator` port contracts (AD-4) are unchanged and ready to implement against whenever this is unblocked — this is a scheduling deferral, not a design gap.
- **`[UPDATED 2026-07-15]` Analytics surfaces** (FR-22, FR-24, FR-25): Capability Map, coverage analytics, and the multi-application executive dashboard are deferred post-V1 — no screen in the current reference prototype's IA serves them. FR-23 (Journey detail) is *not* deferred — it's retained, served inline from the discovery-review flow instead of a standalone screen.
- **`[UPDATED 2026-07-15]` AI-provider-mode configuration UI** (FR-26, FR-27): the Settings screen — the only confirmed V1 entry point for this — is cut from the current reference prototype's IA. Flagged explicitly (not just folded into the deployment-topology deferral below) because FR-26/27 are compliance/data-residency NFRs; confirm before treating the underlying requirement, not just its UI, as deferred.
