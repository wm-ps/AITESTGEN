...
components:
  ...
  icon-button:
    ...
    border: '{colors.border-strong}'
    radius: '{rounded.DEFAULT}'
    # hover-approve/hover-reject variants removed 2026-07-15 — no approve/reject action exists (FR-10/11 cut, see EXPERIENCE.md#Review & Trust Model)
  badge:
    radius: '{rounded.sm}'
    fontSize: 10.5px
    fontWeight: '650'
    ...
    variants:
      ...
      # approved/rejected variants removed 2026-07-15 — no approve/reject status exists (FR-10/11 cut)
  ...
  status-pill:
    ...
    fontSize: 11.5px
    fontWeight: '650'
    note: 'includes a pulse-dot in {colors.signal} while in progress. No "Incomplete" variant — removed 2026-07-15, FR-5 (time budget) cut in full; a Discovery Run only completes or fails, see EXPERIENCE.md#State Patterns'
  card-panel:
    background: '{colors.paper}'
...
- **`{rounded.sm}` (4px)** — the tightest radius, used only for badge/tag chips (`new`, `dupe`, `generated`, etc.) and inline `<code>`-style route/API chips. Small and crisp because these are dense, repeated, high-frequency elements.
...
- **Row overflow menu (`⋯`)** — replaces the earlier fixed icon-button row actions. `[UPDATED 2026-07-15]` Final action sets differ by screen: Discover Journeys rows carry **rename/delete** only (no approve/reject, no edit — see `{EXPERIENCE.md#Review & Trust Model}`); Review Scenarios rows carry **rename/edit/remove** (FR-29, unaffected by the Journey-level cuts).
- **Badges** (`{components.badge}`) — variants seen in the current reference prototype: `Happy Path`, `Negative Path`, `Edge Case` (Review Scenarios), plus `new`/`dupe`/`generated` where still applicable. `[UPDATED 2026-07-15]` `approved`/`rejected` variants removed — no approve/reject status exists. All follow one pattern without exception: a tinted wash background plus saturated text in the *same* hue, never a solid fill.
- **Status pill** (`{components.status-pill}`) — pill-shaped, signal-wash by default, with a small pulsing dot for "in progress" states. `[UPDATED 2026-07-15]` No "Incomplete" state exists anymore — FR-5 (time budget) is removed in full; a Discovery Run only completes or fails (e.g., session expiry). `[GAP]` not confirmed against the new prototype — no in-progress discovery state was reachable during UX review.
...
- **Empty state** — dashed-border panel, a circular green check icon, and a one-line confirmation. `[UPDATED 2026-07-15]` No approve/reject count summary — there's nothing to count since there's no approval step; per Story 3.5, Discover Journeys' empty state is scoped to re-discovery dedup only. Used to close a workflow, not to fill dead space. `[GAP]` exact copy/treatment not reachable in the current reference prototype; retained pending re-verification.
...
| Give Journey rows a per-item overflow (`⋯`) menu for rename/delete, and Scenario rows one for rename/edit/remove (final 2026-07-15 action sets — see `{EXPERIENCE.md#Review & Trust Model}`) | Design or imply merging two Journeys into one, or editing a Journey's composition — deleting a duplicate is still the correct action, not combining or editing it |
