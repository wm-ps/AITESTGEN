...
Extracted from `generation_worker.typecheck` (generation-worker re-imports it
unchanged, via a thin re-export shim) so `apps/workers/recording` — a
different app, not generation-worker — can run the exact same gate on
Codegen's own recorded output before persisting it, without depending on the
whole generation worker for one function."""
...
class TypecheckUnavailable(Exception):
    """node_modules for packages/playwright_typecheck/typecheck isn't
    installed — run `npm install` there."""
