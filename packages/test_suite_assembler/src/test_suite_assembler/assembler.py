...
def _build_playwright_config(base_url: str, *, has_login: bool) -> str:
    ...
    return (
        ...
        "      name: 'setup',\n"
        "      testMatch: /.*\\.setup\\.ts$/,\n"
        "      // `[FIXED]` Playwright's own 30s default test timeout — budgeted for a\n"
        "      // single UI assertion, not a full navigate+login+verify round trip against\n"
        "      // a real remote app under real network latency. Observed live: a real\n"
        "      // Angular app's login alone (goto + fill + submit + settle) routinely used\n"
        "      // most of that 30s on its own, leaving no margin — the setup test hit the\n"
        "      // timeout mid-`storageState()` call, which then threw 'Target page, context\n"
        "      // or browser has been closed' (a symptom of the timeout tearing the context\n"
        "      // down mid-call, not the real cause) and failed every dependent test in the\n"
        "      // suite, since none of them can even start without this step succeeding.\n"
        "      timeout: 60000,\n"
        "    },\n"
        "    {\n"
        "      name: 'authenticated',\n"
...
def _build_auth_setup_script(auth_method: str, login_evidence: LoginPageEvidence | None) -> str:
    ...
    return (
        ...
        f"  const passwordField = {password_field}\n"
        "\n"
        "  // `[FIXED]` Wait for the actual discovered login signal (same locator\n"
        "  // fillCredentials below fills), not `networkidle` — a real SPA with\n"
        "  // background polling/analytics/websockets never goes idle, so that wait\n"
        "  // always burned its full budget without telling us anything about whether\n"
        "  // the login form was ready. Bounded retry (2 attempts) covers a slow first\n"
        "  // navigation without piling on the crawler's much larger, heartbeat-backed\n"
        "  // retry budget — this runs as a bare process with no heartbeat, so its\n"
        "  // budget has to be its own and stay small. Already-authenticated (a reused\n"
        "  // storageState, a session that never expired) short-circuits immediately —\n"
        "  // no wasted wait, no re-login.\n"
        "  let ready = false\n"
        "  let alreadyAuthenticated = false\n"
        "  for (let attempt = 0; attempt < 2 && !ready; attempt++) {\n"
        "    const navigated = await page\n"
        "      .goto(loginUrl, { timeout: 10000 })\n"
        "      .then(() => true)\n"
        "      .catch(() => false)\n"
        ...
        "  if (!ready) {\n"
        "    throw new Error(\n"
        "      `Authentication setup failed: could not reach a login form at ${loginUrl} — ` +\n"
        "      'navigation kept timing out or the discovered password field never became ' +\n"
        "      'visible after 2 attempts. Verify the application is reachable and the ' +\n"
        "      'discovered login page is still correct.',\n"
        "    )\n"
        "  }\n"
        "\n"
        ...
        "\n"
        "    // `[FIXED]` Signal-based, not `networkidle`: wait for the discovered password\n"
        "    // field to actually disappear (the real evidence a login navigation/render\n"
        "    // happened), rather than guessing at network activity. Still `.catch`-guarded —\n"
        "    // a courtesy settle pause, never a hard requirement — the real correctness gate\n"
        "    // is `isAuthenticated` right below.\n"
